quizApp.controller('quizListController',function($scope,$routeParams,$rootScope,$http,$location ){
	console.log('controller OK quiz');
	...
	$scope.saveModal = function(){
		if($scope.modalType == 'add'){
			$http.post("control/quizCtrl.php?action=add",$scope.modal).
			...
			$http.post("control/quizCtrl.php?action=edit",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
			...
			$http.post("control/quizCtrl.php?action=delete",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
	...
	$scope.list = function(){
		...
		$http.get("control/quizCtrl.php?action=list&order="+order+"&limit="+limitPage+"&search="+str).
		success(function(data, status, headers, config) {
	...
	$scope.add = function(){
		...
		$scope.modalType = 'add';
		$scope.modalTitle = dic('Add',$rootScope.langSelected) + ' ' + dic('quiz',$rootScope.langSelected);
		$scope.modal = {};
	}
	...
	$scope.delete = function(item){
		...
		$scope.modalTitle = dic('Delete',$rootScope.langSelected) + ' ' + dic('quiz',$rootScope.langSelected);
		$scope.modal = item;
	}
	...
	$scope.edit = function(item){
		...
		$scope.modalTitle = dic('Edit',$rootScope.langSelected) + ' ' + dic('quiz',$rootScope.langSelected);
		$scope.modal = item;
		dsh(item);
	}
